...
public class BattleDialogBox : MonoBehaviour
{
    ...
    public float timeToAwaitAfterText = 1f;
    ...
    public IEnumerator SetDialog(string message)
    {
        ...
        yield return new WaitForSeconds(timeToAwaitAfterText);
        isWriting = false;
    }
    ...
    public void SelectAction(int selectedAction)
    {
        ...
        {
            actionTexts[i].color = (i == selectedAction
                ? ColorManager.SharedInstance.selectedColor
    ...
    public void SelectMovement(int selectedMovement, Move move)
    {
        for (int i = 0; i < movementTexts.Count; i++)
        {
            movementTexts[i].color = (i == selectedMovement
                ? ColorManager.SharedInstance.selectedColor
                : ColorManager.SharedInstance.defaultColor);
        }
