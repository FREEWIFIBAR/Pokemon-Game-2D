...
public class BattleUnit : MonoBehaviour
{
    ...
    {
        var seq = DOTween.Sequence();
        seq.Append(pokemonImage.DOFade(0, captureTimeAnim));
        seq.Join(transform.DOScale(new Vector3(0.25f, 0.25f, 1f), captureTimeAnim));
        seq.Join(transform.DOLocalMoveY(initialPosition.y + 50f, captureTimeAnim));
    ...
    {
        var seq = DOTween.Sequence();
        seq.Append(pokemonImage.DOFade(1, captureTimeAnim));
        seq.Join(transform.DOScale(new Vector3(1f, 1f, 1f), captureTimeAnim));
